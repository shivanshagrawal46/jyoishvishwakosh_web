...
const MantraTantraContentItem = React.memo(({ content, index, getFirstChar, getIconColor, onTitleClick }) => {
...
MantraTantraContentItem.displayName = 'MantraTantraContentItem'

const MantraTantraPage = ({ language: initialLanguage, setLanguage: setLanguageProp }) => {
  ...
  // Fetch categories on mount - Category ID 8 for Mantra Tantra
  useEffect(() => {
    const loadCategories = async () => {
      try {
        setInitialLoading(true)
        const data = await fetchKoshCategories(8) // Category ID 8 for Mantra Tantra
  ...
  useEffect(() => {
    ...
    const loadContents = async () => {
      ...
      try {
        ...
        const page1 = await fetchKoshContents(8, categoryId, 1) // Category ID 8 for Mantra Tantra
        if (categoryIdRef.current !== categoryId) return
        ...
        for (let i = 2; i <= burstPages; i++) {
          burstPromises.push(fetchKoshContents(8, categoryId, i)) // Category ID 8 for Mantra Tantra
        }
        ...
        if (totalPages > 5) {
          loadBackgroundPages(8, categoryId, 6, totalPages, allInitial) // Category ID 8 for Mantra Tantra
        }
      } catch (err) {
  ...
  return (
    <div className="app kosh-app-fixed">
      ...
      <ServicesStrip language={language} activeService="mantra-tantra" />
...
export default MantraTantraPage
