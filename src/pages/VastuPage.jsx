...
const VastuContentItem = React.memo(({ content, index, getFirstChar, getIconColor, onTitleClick }) => {
...
VastuContentItem.displayName = 'VastuContentItem'

const VastuPage = ({ language: initialLanguage, setLanguage: setLanguageProp }) => {
  ...
  // Fetch categories on mount - Category ID 3 for Vastu
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const loadContents = async () => {
      ...
      try {
        ...
        const page1 = await fetchKoshContents(3, categoryId, 1) // Category ID 3 for Vastu
        if (categoryIdRef.current !== categoryId) return
        ...
        for (let i = 2; i <= burstPages; i++) {
          burstPromises.push(fetchKoshContents(3, categoryId, i)) // Category ID 3 for Vastu
        }
        ...
        if (totalPages > 5) {
          loadBackgroundPages(3, categoryId, 6, totalPages, allInitial) // Category ID 3 for Vastu
        }
      } catch (err) {
  ...
  return (
    <div className="app kosh-app-fixed">
      ...
      <ServicesStrip language={language} activeService="vastu" />
