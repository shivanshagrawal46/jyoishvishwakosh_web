...
const ChalisaAartiContentItem = React.memo(({ content, index, getFirstChar, getIconColor, onTitleClick }) => {
...
ChalisaAartiContentItem.displayName = 'ChalisaAartiContentItem'

const ChalisaAartiPage = ({ language: initialLanguage, setLanguage: setLanguageProp }) => {
  ...
  // Fetch categories on mount - Category ID 5 for Chalisa Aarti
  useEffect(() => {
    const loadCategories = async () => {
      try {
        setInitialLoading(true)
        const data = await fetchKoshCategories(5) // Category ID 5 for Chalisa Aarti
  ...
  useEffect(() => {
    ...
    const loadContents = async () => {
      ...
      try {
        ...
        const page1 = await fetchKoshContents(5, categoryId, 1) // Category ID 5 for Chalisa Aarti
        if (categoryIdRef.current !== categoryId) return
        ...
        for (let i = 2; i <= burstPages; i++) {
          burstPromises.push(fetchKoshContents(5, categoryId, i)) // Category ID 5 for Chalisa Aarti
        }
        ...
        if (totalPages > 5) {
          loadBackgroundPages(5, categoryId, 6, totalPages, allInitial) // Category ID 5 for Chalisa Aarti
        }
      } catch (err) {
  ...
  return (
    <div className="app kosh-app-fixed">
      ...
      <ServicesStrip language={language} activeService="chalisa-aarti" />
...
export default ChalisaAartiPage
