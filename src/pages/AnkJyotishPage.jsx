...
const AnkJyotishContentItem = React.memo(({ content, index, getFirstChar, getIconColor, onTitleClick }) => {
...
AnkJyotishContentItem.displayName = 'AnkJyotishContentItem'

const AnkJyotishPage = ({ language: initialLanguage, setLanguage: setLanguageProp }) => {
  ...
  // Fetch categories on mount - Category ID 4 for Ank Jyotish
  useEffect(() => {
    const loadCategories = async () => {
      try {
        setInitialLoading(true)
        setError(null)
        const data = await fetchKoshCategories(4) // Category ID 4 for Ank Jyotish
        console.log('📋 AnkJyotish categories data received:', data)
        ...
        console.error('❌ Error loading ankjyotish categories:', err)
  ...
  useEffect(() => {
    ...
    const loadContents = async () => {
      ...
      try {
        ...
        const page1 = await fetchKoshContents(4, categoryId, 1) // Category ID 4 for Ank Jyotish
        if (categoryIdRef.current !== categoryId) return
        ...
        for (let i = 2; i <= burstPages; i++) {
          burstPromises.push(fetchKoshContents(4, categoryId, i)) // Category ID 4 for Ank Jyotish
        }
        ...
        if (totalPages > 5) {
          loadBackgroundPages(4, categoryId, 6, totalPages, allInitial) // Category ID 4 for Ank Jyotish
        }
      } catch (err) {
  ...
  return (
    <div className="app kosh-app-fixed">
      ...
      <ServicesStrip language={language} activeService="ank-jyotish" />
