...
const HasthRekhaContentItem = React.memo(({ content, index, getFirstChar, getIconColor, onTitleClick }) => {
...
HasthRekhaContentItem.displayName = 'HasthRekhaContentItem'

const HasthRekhaPage = ({ language: initialLanguage, setLanguage: setLanguageProp }) => {
  ...
  // Fetch categories on mount - Category ID 2 for Hasth Rekha
  useEffect(() => {
    const loadCategories = async () => {
      try {
        setInitialLoading(true)
        setError(null)
        const data = await fetchKoshCategories(2) // Category ID 2 for Hasth Rekha
        console.log('📋 HasthRekha categories data received:', data)
        ...
        console.error('❌ Error loading hasthrekha categories:', err)
  ...
  useEffect(() => {
    ...
    const loadContents = async () => {
      ...
      try {
        ...
        const page1 = await fetchKoshContents(2, categoryId, 1) // Category ID 2 for Hasth Rekha
        if (categoryIdRef.current !== categoryId) return
        ...
        for (let i = 2; i <= burstPages; i++) {
          burstPromises.push(fetchKoshContents(2, categoryId, i)) // Category ID 2 for Hasth Rekha
        }
        ...
        if (totalPages > 5) {
          loadBackgroundPages(2, categoryId, 6, totalPages, allInitial) // Category ID 2 for Hasth Rekha
        }
      } catch (err) {
  ...
  return (
    <div className="app kosh-app-fixed">
      ...
      <ServicesStrip language={language} activeService="hasth-rekha" />
