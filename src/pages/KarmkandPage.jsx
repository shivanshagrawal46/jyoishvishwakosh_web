...
import { fetchKarmkandCategories, fetchKarmkandSubcategories, fetchKarmkandContents } from '../services/api'

// Memoized content item to prevent unnecessary re-renders
const KarmkandContentItem = React.memo(({ content, index, getFirstChar, getIconColor, onTitleClick }) => {
...
KarmkandContentItem.displayName = 'KarmkandContentItem'

const KarmkandPage = ({ language: initialLanguage, setLanguage: setLanguageProp }) => {
  ...
  // Fetch categories on mount - Default category ID is 1
  useEffect(() => {
    const loadCategories = async () => {
      try {
        setInitialLoading(true)
        const data = await fetchKarmkandCategories()
        setCategories(data || [])
        
        // Always select category ID 1 by default
        const defaultCategory = data?.find(cat => cat.id === 1) || data?.[0]
        if (defaultCategory) {
  ...
  useEffect(() => {
    ...
    const loadSubcategories = async () => {
      try {
        ...
        const data = await fetchKarmkandSubcategories(selectedCategory.id)
  ...
  useEffect(() => {
    ...
    // Prevent duplicate loading for same subcategory
    if (subcategoryIdRef.current === subcategoryId) return
    ...
    const loadContents = async () => {
      ...
      try {
        ...
        const page1 = await fetchKarmkandContents(selectedCategory.id, subcategoryId, 1)
        if (subcategoryIdRef.current !== subcategoryId) return
        ...
        for (let i = 2; i <= burstPages; i++) {
          burstPromises.push(fetchKarmkandContents(selectedCategory.id, subcategoryId, i))
        }
        ...
        if (totalPages > 5) {
          loadBackgroundPages(selectedCategory.id, subcategoryId, 6, totalPages, allInitial)
        }
      } catch (err) {
        if (subcategoryIdRef.current === subcategoryId) {
  ...
  const loadBackgroundPages = useCallback(async (categoryId, subcategoryId, startPage, totalPages, existingContents) => {
    ...
    while (currentPage <= totalPages) {
      ...
      for (let p = currentPage; p <= endPage; p++) {
        pagePromises.push(fetchKarmkandContents(categoryId, subcategoryId, p))
      }
  ...
  return (
    <div className="app kosh-app-fixed">
      ...
      <ServicesStrip language={language} activeService="karmkand" />
      
      <main className="kosh-page-main-new">
        <div className="kosh-page-layout karmkand-layout">
          {/* Left Sidebar - Categories */}
          <aside className="kosh-sidebar karmkand-categories-sidebar">
          ...
          <aside className="kosh-sidebar karmkand-subcategories-sidebar">
            <div className="kosh-sidebar-header">
              <h3>{language === 'hindi' ? 'उप-श्रेणियाँ' : 'Subcategories'}</h3>
            </div>
            <div className="kosh-sidebar-categories">
              {subcategories.length > 0 ? (
                subcategories.map((subcategory, index) => (
                  <motion.button
                    key={subcategory.id}
                    className={`kosh-sidebar-category ${selectedSubcategory?.id === subcategory.id ? 'active' : ''}`}
            ...
            <div className="kosh-contents-card">
              <div className="kosh-contents-card-header">
                <h4 className="kosh-contents-card-title">
                  {selectedSubcategory?.name || selectedCategory?.name || (language === 'hindi' ? 'सामग्री' : 'Contents')}
